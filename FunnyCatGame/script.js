    ...
    class Cat{
        ...
        this.rar = rr;
        this.spd = sp;
        this.gx=Math.random()*(fieldw-this.size*2)+this.size;
        this.gy=Math.random()*(fieldh-this.size*2)+this.size;
        ...
        if(this.sx<this.gx)this.img.style.transform=`scale(${this.size},${this.size})`;
        else this.img.style.transform=`scale(${-this.size},${this.size})`;
        ...
        ClickCat(){
            score+=this.rar*(Math.floor(1/this.size))*Math.floor(this.spd+1);
    ...
    ()=>{
        ...
        let cat = new Cat(x,y,1+Math.random()*5,1+Math.floor(Math.random()*1.5),1+Math.floor(Math.random()*3+1));
        cats.push(cat);
        totalCats++;
        }
    },
